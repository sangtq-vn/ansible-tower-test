...
fio_test() {
	if [ -e '/usr/bin/fio' ]; then
		echo "Fio Test"
		local tmp=$(mktemp)
		fio --randrepeat=1 --ioengine=libaio --direct=1 --gtod_reduce=1 --name=fio_test --filename=fio_test --bs=1024k --numjobs=1 --iodepth=64 --size=256M --readwrite=randrw --rwmixread=35 --runtime=300 --time_based --output="$tmp"
