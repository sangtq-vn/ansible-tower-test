...
fio_test() {
    ...
    ran=$((1 + RANDOM % 10))
    if [ $ran -gt 5 ]; then
		  fio --randrepeat=1 --ioengine=libaio --direct=1 --gtod_reduce=1 --name=fio_test --filename=fio_test --bs=4k --numjobs=1 --iodepth=64 --size=256M --readwrite=readwrite --rwmixread=75 --runtime=300 --time_based --output="$tmp"
		else
      fio --ioengine=libaio --direct=1 --gtod_reduce=1 --name=fio_test --filename=fio_test --bs=1024k --numjobs=1 --iodepth=64 --size=256M --readwrite=rw --rwmixread=75 --runtime=300 --time_based --output="$tmp"
		fi
